...
<html lang="en">
  ...
  <select class="form-select" id="departamento" name="code" required>
    <options selected disabled value="">Choose One...</options>
    @foreach ($departamentos as $departamento)
        @if ($departamento->depa_codi == $municipio->depa_codi)
            <option selected value="{{ $departamento->depa_codi }}">{{ $departamento->depa_nomb}}</options>
        @else
            <option value ="{{ $departamento->depa_codi }}">{{ $departamento->depa_nomb }}</options>
        @endif
    @endforeach
</select>
  <div class="mt-3">
